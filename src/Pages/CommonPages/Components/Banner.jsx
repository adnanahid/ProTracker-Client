    ...
    <swiper-container class="mySwiper" navigation="true">
      {slides.map((slide, index) => (
        <swiper-slide key={index}>
          <div className="w-full bg-[#191919] text-gray-200 text-center py-16 px-6 min-h-[600px] flex items-center justify-center md:pt-48">
            <div className="max-w-screen-md">
              <p className="text-sm font-medium mb-4">{slide.infoText}</p>
              <h1 className="text-4xl font-bold mb-4">{slide.title}</h1>
              <p className="text-lg  mb-8">{slide.description}</p>
              <div className="flex justify-center gap-4">
                <Link
                  to={slide.to}
                  className="btn px-6 py-3 bg-white rounded-md md:w-40"
                >
                  {slide.buttonText}
                </Link>
              </div>
